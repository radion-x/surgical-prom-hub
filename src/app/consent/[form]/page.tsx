...
export default function ConsentFormPage() {
  ...
  // Redirect back to home with a thank-you banner so the form "closes"
  router.push(`/?submitted=1&id=${encodeURIComponent(json.id as string)}&form=${encodeURIComponent(params.form)}`);
    } catch (e) {
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <button onClick={() => router.back()} className="text-sm text-blue-700 hover:text-blue-800">← Back</button>
      </div>

      <div className="bg-white shadow-sm ring-1 ring-gray-100 rounded-xl overflow-hidden">
        <div className="px-6 py-5 bg-gradient-to-r from-blue-600/5 to-blue-600/10 border-b border-gray-100">
        ...
        <form className="p-6 grid grid-cols-1 gap-5" onSubmit={(e) => { e.preventDefault(); if (valid) submit(); }}>
          {def.fields.map((f) => (
            <div key={f.name} className="grid gap-1">
              ...
              {f.type === "date" ? (
                <input
                  id={f.name}
                  type="date"
                  className="border border-gray-300 rounded-md px-3 py-2 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required={f.required}
